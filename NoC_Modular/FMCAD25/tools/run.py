...
def permute_ranges(*args) -> tuple[product, int]:
...
def get_specs(path: Path) -> List[SimSpec] | None:
    ...
    for spec in specfile:
        ...
        for var, val in specfile[spec].items():
            ...
            if var == "_type_" and type is None:
                printv(f"Found type variable for {spec} in {path.name}:")
                printv(f"\t_type_ = {val}")
                type = val
...
if __name__ == "__main__":
    ...
    if specfile_path is None:
        print(
            f"{ERR} Simulation specification .toml file is not found. Looked for file at:"
        )
