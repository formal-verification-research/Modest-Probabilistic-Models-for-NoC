...
def run_psn_analysis(
    ...
    assert 1 <= width
    assert 1 <= height
    assert 3 <= width + height # No 1x1 nocs
    ...
    # Create the title
    title = f"{sim_type}_new_prob_{width}x{height}_a{activity_threshold}"
...
def main():
    ...
    # Run the 2x2 model
    with open(script_dir / "modular_sizes.modest", "r") as f:
        model = f.read()

    # Generate unique sizes of NoCs
    for width, height in [(1, 2), (3, 4), (3, 2)]:
        # Resistive nxm Simulations
        for thresh, clk in [(3, 350)]:
        ...
        # Inductive nxm Simulations
        for thresh, clk in [(2,500)]:
