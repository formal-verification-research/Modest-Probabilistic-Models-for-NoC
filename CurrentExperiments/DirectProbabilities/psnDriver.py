...
def generate_incorrect_model(original_model: str, clk: range, type: NoiseType):
    for router in range(0,9):
        for i in clk:
            if type == "Resistive":
                original_model += f"\nproperty r{router}_R_{i} = Pmax(<>[S(clk)<={i}] (noc[{router}].thisActivityTemp >= ACTIVITY_THRESH));"
            elif type == "Inductive":
                original_model += f"\nproperty r{router}_I_{i} = Pmax(<>[S(clk)<={i}] (abs(noc[{router}].thisActivityTemp - noc[{router}].lastActivity) >= ACTIVITY_THRESH));"
    return original_model

def generate_correct_model(original_model: str, clk: range, type: NoiseType):
    for router in range(0,9):
        for i in clk:
            if type == "Resistive":
...
def run_psn_analysis(
    ...
    # Create the title
    title = f"{sim_type}_3x3_a{activity_threshold}_{"correct" if correct else "incorrect"}"
    ...
    for i in chunked_range.chunked_range(0, max_clk+1, stride, batch):
        ...
        if correct:
            model = generate_correct_model(original_model,
                                           i,
                                           sim_type)
        else:
            model = generate_incorrect_model(original_model,
                                             i,
                                             sim_type)
...
def main():
    ...
    # Run the 2x2 model
    try:
        with open(script_dir / "direct_PSN.modest", "r") as f:
            model = f.read()
    except:
    ...
    # Run both correct and incorrect simualtions
