...
def bursty():
    ...
    # Run the 2x2 model
    try:
        with open(script_dir / "modular_corners.modest", "r") as f:
            model = f.read()
    except:
        print("Failed to open model file 'modular_corners.modest'")
        return
    
    # Display output
    print("Running corners model...")
    
    # Setup the output directory
    timestamp = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    output_dir = script_dir / sanitize_filename(f"results_corners_{timestamp}")
    output_dir.mkdir()
